...
export default function SmartEditorPage() {
        ...
        try {
            ...
            const processAndMerge = (newBatch: QuestionData[]) => {
                ...
                newBatch.forEach(newQ => {
                    // Standardize ID: [Page]_[Number]
                    const qNum = newQ.question_number || '0';
                    const pNum = newQ.page || 0;
                    const structuredId = `${pNum}_${qNum}`;

                    const processedQ = { ...newQ, itemId: structuredId };

                    const existingIndex = updated.findIndex(q => q.itemId === structuredId);
                    if (existingIndex > -1) {
                        // Overwrite existing with new AI data
                        updated[existingIndex] = processedQ;
                    } else {
                        // Add new
            ...
            for (const [index, batch] of batches.entries()) {
                ...
                try {
                    ...
                    if (questionText.length > 200) {
                        batchQuestions = await analyzeText(questionText, answerText);
                    } else {
                        const images = await convertPdfToImages(questionFile, batchRange, BATCH_SIZE);
                        batchQuestions = await analyzeImages(images, answerText);
                    }
    ...
    const handleSaveQuestions = async (finalQuestions: QuestionData[]) => {
        try {
            ...
            // Strategy: Full Refresh for this book to ensure DB matches Editor exactly.
            // 1. Delete ALL existing questions for this book
